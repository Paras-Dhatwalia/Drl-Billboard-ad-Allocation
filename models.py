...
class BillboardAllocatorGNN(nn.Module):
    ...
    def forward(self, observations: Dict[str, torch.Tensor], 
        ...
        # Build PyTorch Geometric batch for graph processing
        data_list = [
            Data(x=observations['graph_nodes'][b], 
                 edge_index=observations['graph_edge_links'][b])
            for b in range(batch_size)
        ]
        batch_data = Batch.from_data_list(data_list)
        
        # Encode billboard spatial relationships
        billboard_embeds = self.graph_encoder(batch_data.x.float(), batch_data.edge_index.long())
        billboard_embeds = billboard_embeds.view(batch_size, self.n_billboards, -1)
    ...
    def critic_forward(self, observations: Dict[str, torch.Tensor]) -> torch.Tensor:
        """
        Forward pass for critic network - unchanged as it was already correct.
        """
        observations = preprocess_observations(observations)
        
        batch_size = observations['graph_nodes'].shape[0]
        
        # Build PyTorch Geometric batch
        data_list = [
            Data(x=observations['graph_nodes'][b], 
                 edge_index=observations['graph_edge_links'][b])
            for b in range(batch_size)
        ...
        # Encode billboard features
        billboard_embeds = self.graph_encoder(batch_data.x.float(), batch_data.edge_index.long())
        
        # Global pooling for state value estimation
        pooled = global_mean_pool(billboard_embeds, batch_data.batch)
